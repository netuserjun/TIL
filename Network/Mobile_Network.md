...
## 9장 
802.11 vs 와이파이
둘은 다른 용어지만 사실상 같은 용어. <br>
802.11은 미국의 표준기술. 와이파이는 인증기관. <br>
11은 시장성에서 유럽기술을 압도함. 인텔 주도. <br>
비면허대역. <br>
3가지 물리층 기술 : DS(왈시코드같은 코드 곱하는 방식. 지금은 별로 안씀), FH(랜덤으로 여기저기 뛰어다님), 적외선. <br>

2.4 GHz 대역은 포화상태. 11개 채널이 있지만 서로 겹치는 구조니까 간섭이 무조건 있고. 완전히 쪼개지는 채널은 3개. <br>
밴드폭 20메가 <br>
5기가대역은 2.4보다 멀리는 못가지만 채널도 많고 밴드폭이 넓어서 여유롭다. <br>
<br>
와이파이는 CSMA/CA 방식 사용.<br>
히든노드 문제. <br>
11a 는 처음으로 OFDM 기술 사용. <br>
PCF 가 DCF보다 성능이 좋지는 않다. 현재도 대부분 DCF를 쓴다. PCF는 경쟁이 없다. 근데 비면허대역이라 의미가 없다?<br>
<br>
Feature detection : 근처의 다른 와이파이 기계들이 보내는 비콘 신호를 다 수집해서 누가있나알아냄. <br>
<br>
슬롯 하나는 20마이크로s. 3까지 세기로 했는데 1까지 세고 다른게 통신 시작하면 2개만 더 세면된다. <br>
CTS는 다른 모든 스테이션들이 디코딩을 해서 내용을 알아야한다. 즉 와이파이는 다른 통신을 다 까보니까 보안이 별로안좋다. <br>
CTS에 NAV가 있고 다른 스테이션들은 이 값만큼 카운팅해야한다. <br>
컨텐션윈도우는 백오프크기의 최대값. 문제가 생기면 두 배씩 늘어난다.<br>
컨트롤프레임은 RTS CTS 등등, 관리프레임은 <br>
여러개의 AP로 커버리지를 늘릴수있도록 주소를 4개 가짐. <br>
To DS, 단말이 AP로 데이터를 보내는것. <br>
From DS : 넘어온 데이터가 AP로부터 오는 것.<br>
<br>

### power management
PS모드 파워 세이빙모드 : 단말이 한번씩 깨는 타이밍을 정해. 단말에게 보낼 데이터가 있으면 비콘으로 알림. <br>
11b 는 PS가 좀 비효율적인 방법. <br>
11e는 b를 개선. PS가 개선됏다는것만 기억. <br>


### 무선랜표준
11e는 DCF로 음성과 게임 등 제공. 서비스 품질 개선. <br>
n는 속도 개선. <br>
p는 차량간 직접 통신. 요즘 핫. <br>
s는 매쉬 통신. 무선으로 ap들간에 알아서 통신. 별로 안씀. <br>
ac는 대박. 쓰루풋이 6기가까지 나옴, <br>
ad는 속도가 엄청 빠른데 멀리 못감. 프로젝터와 노트북사이정도 <br>
ae는 ac보완. 와이파이를 많이 설치해도 성능이 떨어지지 않게 하겟다. + 아웃도어서비스 + 파워 세이빙 + 사업자친화적<br>
af는 구글에서 관심을 가짐. TV 대역이 남아도는 대역이 많음. 그 대역에서 와이파이 서비스 <br>
ah는 여전히 관심이 많음. IoT 기술. 900대 비면허대역에서 IoT 서비스 제공. 로라 시그폭스<br>
ax는 이제 나온거. 사업자들이 관심 많음. <br>


### 기술들
11b : 왈시코드처럼 코드를 곱해서 전송. 자기 신호만 유리하게 뽑아낼 수 있다. <br>
데이터율 올리는데 코드곱하는건 비적합. <br>
11g : OFDM 사용. 물리층만 바꾸고 맥층은 b처럼 사용. 속도가 빨라짐.<br>
11e : 물리계층은 그대로두고 맥층만 바꿈. DCF 동작 보완. 서비스의 품질을 차별화. <br>
EDCA : <br>
un스케쥴 : 깨고싶을때 깬다. <br>
11n : 채널본딩 + mimo : 같은데이터(안정성) vs 다른데이터(속도) <br>
2.4 기가헤르츠만. 5기가는 오타. <br>
STBC 알 필요없다. <br>

## LTE 네트워크 아키텍쳐

### Beyond 3G (4G가기직전)
WiMAX 와 LTE, UMB 세 가지 길이 있음. <br>
LTE가 이김. <br>
LTE는 2008, LTE-a는 2010. <br>
3G 4G는 니즈가 있었고 기술도 있었다. <br>
5G는 기술도 없고 니즈도 없다.. <br>

### SAE
UE 단말기 <br>
PDN과 UE 사이를 연결해주는 하나의 흐름, 커넥션을 bearer 이라고 부름.  <br>
음성통화를 하면서 웹서핑을 하면 베어러 두개가 동시에 열려잇는 것. <br>
하나의 망에서 다른 망으로 빠져나가는 곳을 게이트라 부름. <br>
하나의 EPS 베어러는 유선과 무선 구간을 모두 가진다. <br>

### 15쪽. 외우기?
enodeB가 기지국. <br>
p-gw : pdn 게이트웨이. 사업자 망과 외부망을 연결해주는 앵커포인트 <br>
pdn 하위에 다른 게이트들도 있음. serving gateway <br>
PCRF : 우선순위, 요금부여 어떻ㄱ 하는가. <br>
PCEF : PCRF 내의 모듈. 실제로 기능을 하는 놈. <br>
VLR : <br>
MME : 로컬 캐시? 단순 캐시 역할뿐 아니라 제어정보를 만들고 수신하고 해석함. <br>
HSS : HLI 가입자 정보를 저장한 중앙의 DB <br>
LTEUU, S11 인터페이스 아라두기.. <br>

### access network

MME는 non access stratum? <br>
RRM 기지국에서는 radio 
