...
### 안티포렌식 대응
안티포렌식 : 증거물을 차단하려는 행동. <br>
disk wipe : 디스크 영구삭제 프로그램. 하드 모든 섹터 내용을 0으로 만든다. <br>
디가우저 : 자기장을 노출시켜서 기록된 데이터를 파괴. <br>

데이터 복구 기법 회피 기술 <br>
디스크 덮어쓰기 : 삭제된 파일중에 물리적으로 디스크에 남아있는 부분을 덮어쓰고 삭제하는 과정을 반복<br>
임의 문자로 덮어쓰고, 그거의 보수로 덮어쓰고, 다시 임의의 문자로 덮어쓰는걸 7번 반복 <br>

비할당 영역에서 삭제 파일을 복구함으로써 심도 있는 조사 가능. <br>
논리적복구 : 훼손/ 삭제된 데이터 복구..<br>

#### 메타데이터기반 복구
메타데이터의 삭제 플래그를 참조해서 복구. 데이터 위치를 알아내서 복구.<br>
파일 정보를 얻지 못하는 경우 : 파일 자체 정보를 통해 복구, 연속적으로 존재하는 파일 복구는 가능. 조각난 경우는 어려움. <br>

스테가노그래피 : 메시지가 전송되고 있다는 사실, 통신의 존재를 숨기는 기술. <br>
히스토그램 : 영상데이터 분석, 원본과 차이 구별. <br>

#### 클러스터
여러 개의 섹터를 묶은 단위. <br>
기사나 시험에 무조건 나오는 문제임. 꽤 강조. <br>
섹터 단위 입출력은 오래걸리니까 여러 개의 섹터를 묶은 클러스터로 한꺼번에 처리. <br>

#### 슬랙 공간 <br>
잉여 공간. 정보 은닉의 가능성이 있으므로 찾아내는게 중요. <br>
4가지 종류가 있음 시험에 꼭 나옴<br>
RAM slack : 섹터에서 사용되지 않은 곳. 다 0으로 채워져있음. 기억해야해 <br>
file slack : 클러스터 내부 나머지 섹터중에서 안쓰이는 부분. 0으로 채워지진 않는다. 원래 잇던게 보임.<br>
계산하는 것도 시험에 나온다. <br>
file system slack : 파일 시스템 할당 크기와 볼륨 크기간의 차이로 인해 발생되는 공간. <br>
volume slack : 전체 볼륨 크기와 할당된 파티션 크기의 차이로 인해 발생되는 공간 <br>


## 11장 보안관리 
보안 거버넌스 : 조직의 보안을 달성하기 위한 구성원 간의 지배 구조. <br>

구현의 어려움 : <br>
1. 조직 구성의 어려움 : 최고보안책임자, 최고기술책임자, 최고정보책임자의 상하위 배치 결정이 어려움. <br>
2. 성과 측정의 어려움 : 보안에는 비용이 들어가는데 사고가 안나면 성과 측정이 어려움 <br>
3. 조직의 무관심 : 보안에 무관심한 경영진과 구성원. <br>

뒤에는 그냥 읽어보세요.

### 보안 프레임워크
조직 구성원 모두가 전문가는 아니어도 보안 수준을 유지 및 향상하기 위한 체계 <br>
ISMS : 정보보호관리체계. 기업이 민감한 정보를 안전하게 보존하도록 관리할 수 있는 체계적 경영 시스템. <br>
PDCA : 계획,수행,점검,조치 를 반복적으로 수행하는 모델. 이걸로 ISMS를 발전시킬 수 있음. <br>
...
종류 : <br>
규칙으로서 지켜야 할 정책 <br>
하려는 일에 적합한 정책이 없을 때 참고하거나 지키도록 유도하는 정책<br>
정보나 사실을 알리는 정책 <br>

policy 가 제일 상위. 반드시 지켜야하는.. <br>
TCSES가 시험에 나온대. 보안등급 강도 강한거부터 순서대로 옳게 나열한것은? <br>
TCSEC : 보안성만 강조 <br>
ITSEC : 무결성 가용성까지 함께 강조. <br>
지금은 두 개 합쳐서 CC로 바뀜. <br>
보안 회사 제품은 cc인증을 받아야한다. <br>
K-ISMS, IOS 27001 는 기업이나 회사가 정보보호를 잘 하고 있는지 인증을 해주는거. <br>
cc는 제품에 대한 인증 <br>

### 개인정보보호
정의 : <br>
PIMS : 개인정보보호 관리. 거시기. <br>
최근엔 ISMS와 PIMS를 합한 ISMS.P로 합쳐짐. 이거 기억하세요.<br>
